---
layout: default
title: "Curriculum Vitae"
description: "Detailed Personal Information"
password_protected: true  
custom_css: true
---

<div id="protected-content" style="display: none;"></div>

<div id="password-prompt">
  <h2>Protected CV</h2>
  <form id="cv-form">
    <input type="password" id="cv-password" placeholder="Enter password" required>
    <button type="submit">Unlock</button>
  </form>
  <p id="error-message" style="color: red; display: none;">Incorrect password</p>
</div>

<script src="https://cdn.jsdelivr.net/npm/marked/marked.min.js"></script>
<script>
// Obfuscation functions remain the same
// Obfuscation functions with proper error handling
const OBFUSCATED_CV = ``;
const OBFUSCATED_PASSWORD = "Q0ROS0hLcmpBS15GT0JFGhob";
const OBFUSCATED_XOR_KEY = "Kg=="; 

function getXorKey() {
  try {
    return atob(OBFUSCATED_XOR_KEY).charCodeAt(0);
  } catch (e) {
    console.error("XOR key decoding failed:", e);
    return 42; // Fallback key
  }
}

function hexToStr(hex) {
  let str = '';
  // Ensure hex string has even length
  hex = hex.length % 2 !== 0 ? '0' + hex : hex;
  for (let i = 0; i < hex.length; i += 2) {
    const hexByte = hex.substr(i, 2);
    if (!/^[0-9a-fA-F]{2}$/.test(hexByte)) {
      console.error("Invalid hex byte:", hexByte);
      continue;
    }
    str += String.fromCharCode(parseInt(hexByte, 16));
  }
  return str;
}

function xorDecode(data, key) {
  let out = '';
  for (let i = 0; i < data.length; i++) {
    out += String.fromCharCode(data.charCodeAt(i) ^ key);
  }
  return out;
}

function rot13(str) {
  return str.replace(/[a-zA-Z]/g, function(c) {
    const code = c.charCodeAt(0);
    return String.fromCharCode(
      code <= 90 ? ((code - 65 + 13) % 26) + 65 : ((code - 97 + 13) % 26) + 97
    );
  });
}

function decodePassword(obf, key) {
  try {
    const xorred = atob(obf);
    return xorDecode(xorred, key);
  } catch (e) {
    console.error("Password decoding failed:", e);
    return "";
  }
}

function decodeCV(obf, key) {
  try {
    console.log("Step 1: Hex to string");
    const hexStr = hexToStr(obf);
    console.log("Hex result:", hexStr);
    
    console.log("Step 2: XOR decode");
    const xorred = xorDecode(hexStr, key);
    console.log("XOR result:", xorred);
    
    console.log("Step 3: ROT13");
    const rotated = rot13(xorred);
    console.log("ROT13 result:", rotated);
    
    console.log("Step 4: Base64 decode (first pass)");
    const b64_1 = atob(rotated);
    console.log("First B64 result:", b64_1);
    
    console.log("Step 5: Base64 decode (final)");
    const decoded = atob(b64_1);
    console.log("Final decoded:", decoded);
    
    return decoded;
  } catch (e) {
    console.error("CV decoding failed at step:", e);
    throw e;
  }
}

document.getElementById('cv-form').addEventListener('submit', function(e) {
  e.preventDefault();
  const input = document.getElementById('cv-password').value.trim();
  const xorKey = getXorKey();
  
  console.log("Attempting to decode with key:", xorKey);
  const correct = decodePassword(OBFUSCATED_PASSWORD, xorKey);
  
  if (!correct) {
    document.getElementById('error-message').textContent = "System error - please refresh";
    document.getElementById('error-message').style.display = 'block';
    return;
  }

  if(input === correct) {
    try {
      console.group("Decoding CV content");
      const decoded = decodeCV(OBFUSCATED_CV, xorKey);
      console.groupEnd();
      
      document.getElementById('protected-content').innerHTML = marked.parse(decoded);
      document.getElementById('protected-content').style.display = 'block';
      document.getElementById('password-prompt').style.display = 'none';
    } catch(err) {
      console.error("Full decoding error:", err);
      document.getElementById('error-message').textContent = 
        "Decoding failed - check console for details";
      document.getElementById('error-message').style.display = 'block';
    }
  } else {
    document.getElementById('error-message').textContent = "Incorrect password";
    document.getElementById('error-message').style.display = 'block';
  }
});
</script>